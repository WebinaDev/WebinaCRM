<?php
/**
 * Accounting Person Category model (دسته‌بندی اشخاص).
 *
 * @package WebinoCRM
...
/**
 * Class WebinoCRM_Accounting_Person_Category
 */
class WebinoCRM_Accounting_Person_Category {

	const TABLE = 'webinocrm_accounting_person_categories';
	...
	/**
	 * Get root categories or children of a parent.
	 *
	...
	/**
	 * Delete category (only if no children and no persons using it).
	 *
	 * @param int $id Category id.
	 * @return bool
	 */
	public static function delete( $id ) {
		...
		$persons_table = $wpdb->prefix . 'webinocrm_accounting_persons';
		$count         = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $persons_table WHERE category_id = %d", $id ) );
		if ( $count > 0 ) {
			return false;
		}
		$table = self::get_table();
