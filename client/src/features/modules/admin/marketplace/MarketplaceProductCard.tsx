...
import type { MarketplaceModule } from "@/api/marketplace"
...
export function MarketplaceProductCard({ module: m, statusBusy, onStatusToggle, onDelete }: Props) {
  ...
  const priceLabel = m.is_free
    ...
    : `${m.price?.toLocaleString() ?? 0} ${m.currency ?? "IRT"}`

  const categoryLabel = m.category_name?.trim()
    ? m.category_name
      ...
      <CardContent className="flex h-full flex-col pt-10">
        <div className="mb-3 flex flex-col items-center text-center">
          ...
          <p className="text-muted-foreground mt-1 text-xs" dir="ltr">
            {t("pages.marketplace.version")}: {m.latest_version ?? m.version ?? "—"}
          </p>
        </div>
        ...
        <div className="mb-3 flex flex-wrap justify-center gap-1">
          {m.is_core ? (
            <Badge variant="secondary" className="text-xs">
              {t("pages.marketplace.coreProduct")}
            </Badge>
          ) : null}
          {m.is_builtin && !m.is_core ? (
            <Badge variant="outline" className="text-xs">
              {t("pages.marketplace.bundledWithDashboard")}
            </Badge>
          ) : null}
          {m.parent_name ? (
            <Badge variant="outline" className="text-xs">
              {t("pages.marketplace.submoduleOf", { name: m.parent_name })}
            </Badge>
          ) : null}
        </div>
        ...
        <div className="border-border flex items-center justify-between gap-2 border-t pt-3">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor={`mp-status-${m.id}`} className="text-muted-foreground text-xs">
              {m.status === "active"
                ? t("pages.marketplace.statusActive")
                : t("pages.marketplace.statusInactive")}
            </Label>
          </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                  <Link to={`/marketplace/modules/${m.id}`}>
                    <Settings className="h-4 w-4" />
                    <span className="sr-only">{t("pages.marketplace.productCard.manage")}</span>
                  </Link>
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("pages.marketplace.productCard.manage")}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                  <Link to={`/marketplace/modules/${m.id}#releases`}>
                    <Tag className="h-4 w-4" />
                    <span className="sr-only">{t("pages.marketplace.productCard.releases")}</span>
                  </Link>
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("pages.marketplace.productCard.releases")}</TooltipContent>
            </Tooltip>
            {repoUrl ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                    <a href={repoUrl} target="_blank" rel="noreferrer">
                      ...
                      <span className="sr-only">{t("pages.marketplace.productCard.openRepo")}</span>
                    </a>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{t("pages.marketplace.productCard.openRepo")}</TooltipContent>
              </Tooltip>
            ) : null}
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <span className="sr-only">{t("pages.marketplace.productCard.delete")}</span>
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t("pages.marketplace.productCard.delete")}</TooltipContent>
            </Tooltip>
          </div>
        </div>
      </CardContent>
