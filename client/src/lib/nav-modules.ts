...
export function moduleNavTitle(t: TFunction, id: string, fallback: string): string {
  ...
  return t(`layout.${layoutKey}`, { defaultValue: fallback })
}

function mapNavChild(c: DashboardModule, parent: DashboardModule, t: TFunction): NavMainChild {
...
function mapModuleChildToNavSection(c: DashboardModule, parent: DashboardModule, t: TFunction): NavMainSection {
  const nested = validChildren(c)
  if (nested.length > 0) {
    return {
      kind: 'group',
